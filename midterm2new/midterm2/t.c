...
{
  int seconds;
  //printf("enter a timer value: ");
  seconds =geti();
  tqe_enqueue(seconds, running);
  ksleep(running);
  tswitch();
}
...
int childtree (PROC *parent, PROC *inchild)
{
  if (!(parent->child))
  {
    parent->child=inchild;
    parent->child->parent=parent;
  }
  else
  {
    PROC *tmp = parent->child;
    while(tmp->sibling)
    {
      tmp = tmp->sibling;
    }
    tmp->sibling=inchild;
    tmp->sibling->parent=parent;
  }
}
...
int do_wait()
{
  int pid, status;
  pid = kwait(&status);
  printf("Wait: Proc %d Zombiechild %d\n", running->pid, pid);
}
